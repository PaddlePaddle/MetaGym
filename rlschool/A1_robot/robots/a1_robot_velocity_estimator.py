...
import sys
sys.path.append("../")
from rlschool.A1_robot.robots.moving_window_filter import MovingWindowFilter
...
class VelocityEstimator:
  ...
  def update(self, robot_state,robot):
    """Propagate current state estimate with new accelerometer reading."""
    delta_time_s = robot.time_step
    sensor_acc = np.array(robot_state["IMU"][3:])
    base_orientation = robot.GetBaseOrientation()
    rot_mat = robot.pybullet_client.getMatrixFromQuaternion(
        base_orientation)
    ...
    for leg_id in range(4):
      if foot_contact[leg_id]:
        ...
        joint_velocities = robot_state["MotorAngleAcc"][12+leg_id *
                                                       3:12+(leg_id + 1) * 3]
